...
async function getDoctorLogs() {
  const coll = await collection(db, "users");
  const q = await query(coll);
  const docs = await getDocs(q);
  currentUser.logs = [];
  docs.forEach((item) => {
    currentUser.logs.push(item.data());
  });

  const realLogs = [];
  currentUser.logs = currentUser.logs.filter((item) => {
    if (item.logs)
      item.logs.forEach((log) => {
        if (log.doctorId === auth.currentUser.uid) realLogs.push({...log,name:item.name});
      });
  });

  currentUser.logs = realLogs;
  return "HI";
}
...
function handlePageButn() {
  if (currentUser.userType === "doctor") {
    ...
    document.getElementById("userText").textContent = "patient Name";
...
const displayInbox = () => {
  ...
  for (let data of serverData) {
    ...
    msgDiv.append(H5);
    msgDiv.append(butn);
    butn.addEventListener("click", () => infoBrief(data.uid,data.name));
...
const getHistory = (firebaseData) => {
  ...
  div1.innerText = firebaseData["message"];
  div2.innerText = firebaseData["name"] || firebaseData["doctorName"]  
  div3.innerText = firebaseData["status"];
...
async function showHistory() {
  ...
  currentUser.logs.forEach((item) => getHistory(item));
}
...
const statusFunction = async (num) => {
  ...
  const docRef1 = doc(db, "users", currentPatient);
